...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Features Section */}
      <section id="features" className="py-20 px-6 bg-[#1a1d23] overflow-hidden relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature, index) => {
              ...
              return (
                <div 
                  key={index}
                  className="relative bg-[hsl(220,13%,9%)] border border-[hsl(220,13%,18%)] rounded-lg overflow-hidden group hover:border-primary/50 transition-all duration-300 flex flex-col"
                  style={{ minHeight: '20em' }}
                >
                  {/* Content Container */}
                  <div className="relative p-6 flex flex-col h-full">
                    ...
                    <div className={`grid grid-cols-12 gap-px mb-3 ${topGridClass}`}>
                      {Array.from({ length: 12 }).map((_, i) => (
                        <div 
                          key={`top-${i}`}
                          className={topGridClass ? `grid-box-animated grid-box-${i}` : 'h-4 bg-white/5 group-hover:bg-white/10 transition-colors'}
                        />
                      ))}
                    </div>
                    ...
                    {/* Description between grids - NOW WITH ANIMATION */}
                    <div className="mb-4">
                      <p className={`text-base leading-relaxed ${descriptionClass}`} style={{ color: 'rgb(156, 163, 175)' }}>
                        {feature.description}
                      </p>
                    </div>
                    ...
                    <div className={`grid grid-cols-12 gap-px mt-auto ${bottomGridClass}`}>
                      {Array.from({ length: 12 }).map((_, i) => (
                        <div 
                          key={`bottom-${i}`}
                          className={bottomGridClass ? `grid-box-animated grid-box-${i}` : 'h-4 bg-white/5 group-hover:bg-white/10 transition-colors'}
                        />
                      ))}
                    </div>
                  </div>
